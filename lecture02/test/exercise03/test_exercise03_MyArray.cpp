...
#include <lecture02/exercise03/MyArray.h>
...
TEST (ValueType, Value) {
    MyArray<int, 50> uut1;
    uut1.fill(3);
    MyArray<int, 50>::value_type m = uut1[0];
    EXPECT_EQ(3, m);
}

TEST (ValueType, ValueAccu) {
    MyArray<int, 50> uut1;
    uut1.fill(3);
    auto m = myAccumalation(uut1);
    EXPECT_EQ(150, m);
}

TEST (ValueType, ValueAccuVector) {
    std::vector<int> uut1;
    uut1.assign(10, 4);
    auto m = myAccumalation(uut1);
    EXPECT_EQ(40, m);
