...
#include <lecture06_variant/exercise01/Variance.h>
#include <sstream>
#include <string>
#include <iostream>
...
TEST (NostreamUserdiffend, Variant){
    IAmUserDefined MeToo;
    boost::variant<int, std::string, IAmUserDefined> myVariant(MeToo);
    std::stringstream stream;

    stream << boost::get<IAmUserDefined>(myVariant).IAmSmallAndHappy;
    std::string map = stream.str();

    EXPECT_EQ(map ,"255");
}

TEST (NostreamUserdiffendSizeT, Variant){
    IAmUserDefined MeToo;
    boost::variant<int, std::string, IAmUserDefined> myVariant(MeToo);
    std::stringstream stream;

    stream << boost::get<IAmUserDefined>(myVariant).ICanBeBig;
    std::string map = stream.str();

    EXPECT_EQ(map ,"123456");
}

TEST (NostreamString, Variant){
    boost::variant<int, std::string, IAmUserDefined> myVariant(std::string("boy"));
    std::stringstream stream;

    stream << boost::get<std::string>(myVariant);
    std::string map = stream.str();
...
TEST (StreamUserDefined, Variant){
    IAmUserDefined MeToo;
    boost::variant<int, std::string, IAmUserDefined> myVariant(MeToo);
    std::stringstream stream;
    ...
    EXPECT_EQ(map ,"ICanBeBig: 123456 IAmSmallAndHappy 255 ICanBeNegative -1");
}
