...
#include <lecture01/exercise03/SharedPtr.h>
...
TEST_F (NormalDestructor, OldPointerShouldDieOnAssignment) {
    ...
    uut2 = uut1;
    EXPECT_EQ(200, this->status);
    EXPECT_EQ(301, status2);
}
...
class DeleterFunction : public ::testing::Test {
protected:
    virtual void SetUp() {
        this->value = new Dummy(&this->status);
    }
...
TEST_F (DeleterFunction, DelterWasRun) {
    MyDeleter del;
    SharedPtr<Dummy> uut1(this->value, new Helper<Dummy, MyDeleter>(del));
    SharedPtr<Dummy> uut2(uut1);
    uut1.Reset();
    EXPECT_EQ(200, this->status);
    uut2.Reset();
    EXPECT_EQ(202, this->status);
}
